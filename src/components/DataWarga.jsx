...
export default function DataWarga({ totalKK, totalHidup, totalMeninggal, wargaList = [] }) {
  ...
  return (
    <section
      id="data-warga"
      className="pt-24 sm:pt-28 pb-16 bg-white border-b border-slate-100"
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-8 mb-12 sm:mb-16">
          ...
          {/* Card 1: Total KK */}
          <div className="group relative bg-slate-50 border border-slate-200/60 rounded-2xl sm:rounded-3xl p-5 sm:p-8 shadow-sm transition-all duration-300 hover:shadow-md">
            ...
            <div className="mt-6 pt-4 border-t border-slate-200/50 flex items-center justify-between text-xs text-slate-500">
              ...
              <span className="font-bold text-emerald-600">± {rataRataAnggotaKK} Orang</span>
            </div>
          </div>

          {/* Card 2: Penduduk Hidup */}
          <div className="group relative bg-slate-50 border border-slate-200/60 rounded-3xl p-8 shadow-sm transition-all duration-300 hover:shadow-md hover:scale-[1.01]">
            <div className="flex justify-between items-start">
              <div className="space-y-2">
                <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Penduduk Hidup</span>
                <span className="block text-4xl font-black text-emerald-600">{totalPenduduk}</span>
                <p className="text-xs text-slate-500">Warga berstatus aktif tinggal menetap/kontrak</p>
              </div>
              <div className="p-3 bg-emerald-500/10 text-emerald-600 rounded-2xl">
          ...
          <div className="group relative bg-slate-50 border border-slate-200/60 rounded-3xl p-8 shadow-sm transition-all duration-300 hover:shadow-md hover:scale-[1.01]">
            <div className="flex justify-between items-start">
              <div className="space-y-2">
                ...
                <span className="block text-4xl font-black text-slate-600">{totalMeninggal}</span>
                <p className="text-xs text-slate-500">Kumulatif data kematian warga (arsip RT)</p>
              </div>
            ...
            <div className="mt-6 pt-4 border-t border-slate-200/50 flex items-center justify-between text-xs text-slate-500">
              ...
              <span className="font-bold text-slate-500">Tercatat di Kelurahan</span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          ...
          <div className="lg:col-span-6 bg-slate-50 border border-slate-200/60 rounded-3xl p-8 shadow-sm flex flex-col justify-between">
            <div>
              <h3 className="font-extrabold text-slate-900 text-lg">Distribusi Jenis Kelamin</h3>
              <p className="text-xs text-slate-500 mt-1">Pembagian total penduduk hidup berdasarkan gender</p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-8 py-6 my-auto">
              <div className="relative w-40 h-40">
                <svg viewBox="0 0 36 36" className="w-full h-full transform -rotate-90">
                  {/* Background Track */}
                  <circle cx="18" cy="18" r="15.915" fill="none" stroke="#e2e8f0" strokeWidth="3" className="" />
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className="text-2xl font-black text-slate-800">{totalPenduduk}</span>
              ...
              <div className="space-y-3 flex-shrink-0">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="block text-xs font-semibold text-slate-700">Laki-Laki ({pctLaki}%)</span>
                    <span className="block text-sm font-bold text-slate-900">{dataDemografiGender.lakiLaki} Jiwa</span>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <div className="w-4 h-4 rounded-md bg-sky-500"></div>
                  <div>
                    <span className="block text-xs font-semibold text-slate-700">Perempuan ({pctPerempuan}%)</span>
                    <span className="block text-sm font-bold text-slate-900">{dataDemografiGender.perempuan} Jiwa</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* Note */}
            <div className="pt-4 border-t border-slate-200/50 text-[11px] text-slate-400 text-center flex items-center justify-center gap-1.5">
          ...
          <div className="lg:col-span-6 bg-slate-50 border border-slate-200/60 rounded-3xl p-8 shadow-sm flex flex-col justify-between">
            <div>
              <h3 className="font-extrabold text-slate-900 text-lg">Kelompok Rentang Usia</h3>
              <p className="text-xs text-slate-500 mt-1">Komposisi penduduk berdasarkan tahapan usia</p>
            </div>
              ...
              {dataDemografiUsia.map((item, idx) => (
                <div key={idx} className="space-y-2">
                  <div className="flex justify-between text-xs sm:text-sm font-semibold">
                    ...
                    <span className="text-slate-900 font-bold">{item.count} Warga ({item.percentage}%)</span>
                  </div>
                  {/* Track */}
