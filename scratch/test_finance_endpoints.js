async function testFinance() {
  ...
  try {
    ...
    if (token) {
      // Test GET tracking
      const trackRes = await fetch('http://172.20.32.115:3333/admin/finance/tracking?month=8&year=2026', {
      ...
      const manualRes = await fetch('http://172.20.32.115:3333/admin/finance/manual-payment', {
        method: 'POST',
        headers: { 
      ...
      const trackRes2 = await fetch('http://172.20.32.115:3333/admin/finance/tracking?month=8&year=2026', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
