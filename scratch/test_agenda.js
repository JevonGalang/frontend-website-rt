...
async function runTests() {
  try {
    ...
    res = await fetch('http://localhost:3333/admin/agenda', {
      method: 'POST',
      headers: {
    ...
    if (agendaId) {
      ...
      res = await fetch(`http://localhost:3333/admin/agenda/${agendaId}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${rtToken}`
    ...
    res = await fetch('http://localhost:3333/resident/agenda', {
